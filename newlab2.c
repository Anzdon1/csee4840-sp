...
#define VIEW_CAP (MAX_COLS * INPUT_ROWS)   /* 64 * 2 = 128 */
...
{
    /* HARD LIMIT: input window is 2 rows */
    if (input_len >= VIEW_CAP)
        return;

    /* also keep within buffer */
...
static void *net_thread(void *arg)
{
    ...
    while (1) {
        ...
        /* Properly lock display while updating framebuffer buffers */
        ...
        redraw_recv();
        /* Keep input cursor visible after recv redraw */
        redraw_input();
